...
func _draw() -> void:
	...
	for x in range(grid_dimensions.x + 1):
		var x_pos = start_x + (x * tile_size)
		draw_line(Vector2(x_pos, start_y), Vector2(x_pos, start_y + height), grid_color, thickness)

	# Disegna righe orizzontali
	for y in range(grid_dimensions.y + 1):
		var y_pos = start_y + (y * tile_size)
		draw_line(Vector2(start_x, y_pos), Vector2(start_x + width, y_pos), grid_color, thickness)
